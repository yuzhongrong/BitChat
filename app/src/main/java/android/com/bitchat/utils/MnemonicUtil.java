...
//    private static final List<String> WORD_LIST = populateWordList();
   private static final List<String> WORD_LIST =populateWordListForAndroid(BaseApplication.getContext());


    /**
     * The item_mnemonic must encode entropy in a multiple of 32 bits. With more entropy security is
     ...
     * c_11 bits, each encoding a number from 0-2047, serving as an index into a wordlist. Finally,
     * we convert these numbers into words and use the joined words as a item_mnemonic sentence.
     *
     * @param initialEntropy The initial entropy to generate item_mnemonic from
     * @return The generated item_mnemonic
    ...
    /**
     * To create a binary seed from the item_mnemonic, we use the PBKDF2 function with a
     * item_mnemonic sentence (in UTF-8 NFKD) used as the password and the string "item_mnemonic"
     ...
     *
     * @param mnemonic The input item_mnemonic which should be 128-160 bits in length containing
     *                 only valid words
    ...
    public static byte[] generateSeed(String mnemonic, String passphrase) {
        ...
        String salt = String.format("item_mnemonic%s", passphrase);
    ...
    private static List<String> populateWordListForAndroid(Context context) {
//        URL url = Thread.currentThread().getContextClassLoader()
//                .getResource("en-item_mnemonic-word-list.txt");



        try {
            InputStream inputStream =context.getAssets().open("en-mnemonic-word-list.txt");
            String path = new String(InputStreamToByte(inputStream ));



            return readAllData1(path);
        } catch (Exception e) {
            return Collections.emptyList();
        }
    }
    ...
    public static List<String> readAllData1(String content) throws IOException {
        List<String> data1= Arrays.asList(content.split("\n"));
