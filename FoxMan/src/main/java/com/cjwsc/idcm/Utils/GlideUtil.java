...
public final class GlideUtil extends AppGlideModule {
    ...
    public static void loadImageViewNoGif(Context mContext, String path, ImageView mImageView) {
        GlideApp.with(mContext).asBitmap().load(path).diskCacheStrategy(DiskCacheStrategy.ALL).into(mImageView);
    }
    //默认加载
    ...
    //默认加载
    public static void loadImageView(Context mContext, @SuppressLint("SupportAnnotationUsage") @DrawableRes Bitmap res, ImageView mImageView) {
        GlideApp.with(mContext).load(res).into(mImageView);
    }
...
//        GlideApp.with(mContext).load(path).animate(anim).into(mImageView);
    ...
    public static void loadImageViewContentWithSizeFixRatio(Context mContext, String path, float ratioX, float ratioY, int width, int height, ImageView imageView, RelativeLayout.LayoutParams layoutParams, back b) {
        GlideApp.with(mContext).load(path).override(width, height).listener(new RequestListener<Drawable>() {
            @Override
            public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Drawable> target, boolean
                    isFirstResource) {
                    ...
                    public void run() {
                        if (resource != null) {
                            ...
                            if (path.endsWith(".gif")) {
                                Glide.with(mContext).asGif().load(path).into(imageView);
                                ...
                                if (intrinsicWidth / intrinsicHeight > ratioX) {//超过设置的宽高比
                                    GlideApp.with(mContext).load(path).override(width, (int) (height * .6 + .5f)).centerCrop().into(imageView);
                                    layoutParams.width = width;
                                    layoutParams.height = (int) (height * .6 + .5f);
                                } else if (intrinsicHeight / intrinsicWidth > ratioY) {//超过设置的高宽比
                                    GlideApp.with(mContext).load(path).override((int) (width * .6 + .5f), (int) (height * 1.2 + .5f)).centerCrop().into(imageView);
                                    layoutParams.width = (int) (width * .6 + .5f);
                                    layoutParams.height = (int) (height * 1.2 + .5f);
                                } else {
                                    GlideUtil.loadImageViewSize(mContext, path, width, height, imageView);
                                    layoutParams.height = intrinsicHeight;
                                    layoutParams.width = intrinsicWidth;
    ...
    public static void loadImageViewContentWithSizeFixRatio(Context mContext, Bitmap path, float ratioX, float ratioY, int width, int height, ImageView imageView, RelativeLayout.LayoutParams layoutParams, back b) {
        GlideApp.with(mContext).load(path).override(width, height).listener(new RequestListener<Drawable>() {
            @Override
            public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Drawable> target, boolean
                    isFirstResource) {
                    ...
                    public void run() {
                        if (resource != null) {
                            ...
                            if (intrinsicWidth / intrinsicHeight > ratioX) {//超过设置的宽高比
                                ...
                                GlideApp.with(mContext).load(path).override(width,height).into(imageView);
                                layoutParams.height = intrinsicHeight;
                                layoutParams.width = intrinsicWidth;
                            }
                        }
